...
do
	...
	hadoop jar hdfsrs/experiments/stephen/MultipleFileLatencyExperiments/MultipleFileLatency.jar setup $file_size /node1file$i.txt
 	if [ $num_clients -gt 1 ]
 	then
		echo $file_size' written to node2file'$i'.txt'
		hadoop jar hdfsrs/experiments/stephen/MultipleFileLatencyExperiments/MultipleFileLatency.jar setup $file_size /node2file$i.txt
	fi
done
