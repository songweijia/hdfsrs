...
//		.size       		= 2147483648,
	...
	if(!data.servername){
		...
		printf("PID=%d | port=%d | ib_port=%d | size=%ld | tx_depth=%d | sl=%d |\n",
...
static struct app_context *init_ctx(struct app_data *data)
{
	...
	TEST_Z(ctx->context = ibv_open_device(data->ib_dev),
        ...
        ibv_query_device(ctx->context,&dattr);
        printf("max_mr_size=%ld\n",dattr.max_mr_size);
        printf("max_sge=%d\n",dattr.max_sge);
        printf("max_sge_rd=%d\n",dattr.max_sge_rd);
        printf("max_cq=%d\n",dattr.max_cq);
        printf("max_cqe=%d\n",dattr.max_cqe);
...
gettimeofday(&tv2,NULL);
      ...
      if (wc.status == IBV_WC_SUCCESS) {
        ...
        printf("Throughput:%.3fMBps\n",((double)ctx->size)/(((tv2.tv_sec-tv1.tv_sec)*1000000)+tv2.tv_usec-tv1.tv_usec));
      }
}	
