...
static void test_odp_support(struct ibv_context *context) {
  //test for ODP
  struct ibv_exp_device_attr dattr;
  dattr.comp_mask = IBV_EXP_DEVICE_ATTR_ODP | IBV_EXP_DEVICE_ATTR_EXP_CAP_FLAGS;
  TEST_NZ(ibv_exp_query_device(context, &dattr), "Could not query device attributes.");
  if (dattr.exp_device_cap_flags & IBV_EXP_DEVICE_ODP) {
    printf("Bingo! ODP is supported!\n");
    printf("rc_odp_caps = 0x%x\n",dattr.odp_caps.per_transport_caps.rc_odp_caps);
    printf("uc_odp_caps = 0x%x\n",dattr.odp_caps.per_transport_caps.uc_odp_caps);
    printf("ud_odp_caps = 0x%x\n",dattr.odp_caps.per_transport_caps.ud_odp_caps);
    printf("dc_odp_caps = 0x%x\n",dattr.odp_caps.per_transport_caps.dc_odp_caps);
    printf("xrc_odp_caps = 0x%x\n",dattr.odp_caps.per_transport_caps.xrc_odp_caps);
    printf("raw_eth_odp_caps = 0x%x\n",dattr.odp_caps.per_transport_caps.raw_eth_odp_caps);
    if (dattr.odp_caps.general_odp_caps & IBV_EXP_ODP_SUPPORT_IMPLICIT) {
      printf("Implicity ODP is supported!\n");
    }
  } else {
    printf("ODP is not supported in with this setup, please check the hardware, driver, and kernel.\n");
  }
}
...
static void init_ctxt(int page_size){
  ...
  in.exp_access = IBV_EXP_ACCESS_ON_DEMAND | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ;
  in.comp_mask = 0;
  TEST_Z(ctxt.mr = ibv_exp_reg_mr(&in), "Could not allocate mr with ibv_exp_reg_mr.");

#ifdef PREFETCHING_VERB
  struct ibv_exp_prefetch_attr prefetch_attr;
  prefetch_attr.flags = IBV_EXP_PREFETCH_WRITE_ACCESS;
  prefetch_attr.addr = ctxt.pages;
  prefetch_attr.length = page_size*MAX_PAGE;
  prefetch_attr.comp_mask = 0;
  ibv_exp_prefetch_mr(ctxt.mr, &prefetch_attr);
#endif
