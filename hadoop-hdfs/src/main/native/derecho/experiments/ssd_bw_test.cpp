...
int main (int argc, char *argv[]) {
  ...
  bool done = false;
  auto stability_callback = [&num_messages, &done, &num_nodes, &fd, &buf, &buffer_size, num_last_received=0u] (int sender_id, long long int index, char *msg_buf, long long int msg_size) mutable {
  ...
  derecho::ManagedGroup managed_group(GMS_PORT, node_addresses, node_rank, server_rank, buffer_size, stability_callback, block_size);
