...
int main () {
  ...
  // int fd = open("messages",O_RDWR|O_CREAT|O_DIRECT|O_SYNC,S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
  int fd = open("messages", O_WRONLY | O_CREAT | O_DIRECT); // also tried O_SYNC: O_WRONLY | O_CREAT | O_DIRECT | O_SYNC
  if (fd < 0) {
